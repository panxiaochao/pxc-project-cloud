...
public class SysTenantPackageApi {
	...
	public R<PageResponse<SysTenantPackageQueryResponse>> page(RequestPage requestPage,
			SysTenantPackageQueryRequest queryRequest) {
		return R.ok(sysTenantPackageAppService.page(requestPage, queryRequest));
	}
	...
	@Operation(summary = "获取租户套餐下拉菜单", description = "获取租户套餐下拉菜单", method = "GET")
	@GetMapping(value = "/selectPackages")
	public R<List<Select<String>>> selectPackages() {
		return R.ok(sysTenantPackageAppService.selectPackages());
	}
