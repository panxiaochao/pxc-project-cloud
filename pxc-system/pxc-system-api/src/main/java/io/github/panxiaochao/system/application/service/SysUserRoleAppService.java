...
public class SysUserRoleAppService {
	 ...
	 * @return 分页数组响应实体
	 */
	public PageResponse<SysUserRoleQueryResponse> page(RequestPage requestPage, SysUserRoleQueryRequest queryRequest) {
	...
	public R<Void> save(SysUserRoleCreateRequest sysUserRoleCreateRequest) {
		if (StringUtils.hasText(sysUserRoleCreateRequest.getRoleId())) {
			// 以,分割roleId为字符串数组
			String[] roleIds = StringUtils.tokenizeToStringArray(sysUserRoleCreateRequest.getRoleId(),
					StringPools.COMMA);
